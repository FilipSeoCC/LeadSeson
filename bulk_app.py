...
def enrich_leads_with_industry_lookup(leads):
    """Add lead_reason/call_script (from the live per-record sugerowana_akcja, single
    source of truth in seasonal_signal.build_seasonal_leads) plus recommended_product
    from the per-branża config table (seasonality_matrix.lookup_by_industry) — the two
    schemes solve different problems (dynamic per-lead text vs. industry-templated
    product pitch) so both are kept, side by side."""
    if leads.empty:
        return leads
    result = leads.copy()
    result["lead_reason"] = result.get("sugerowana_akcja", "")
    result["call_script"] = result["lead_reason"]
    cache = {}
    def _product(branza):
        if branza not in cache:
            cache[branza] = lookup_by_industry(branza) if branza and branza != "Nieokreślona" else {}
        return cache[branza].get("recommended_product", "")
    result["recommended_product"] = result.get("branza_glowna", "").map(_product)
    return result
...
def render_leads_view():
    ...
    leads = enrich_leads_with_industry_lookup(leads)
...
""",
        ...
        preview_cols = [
            ...
            "score_gotowosci", "mrr", "lead_reason", "next_action", "liczba_umow_klienta",
        ]
